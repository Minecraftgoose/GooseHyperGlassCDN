...
import { gooseCenter, gooseBack, gooseBtn, gooseGlass, gooseRect, gooseText } from './helpers'
...
export function buildDialog(
  ...
  // 内容可配：不传 dialogConfig 时回退原版写死文案（Dialog Title / gooseLorem / Cancel / Okay）
  const dlgTitle = dialogConfig?.title ?? 'Dialog Title'
  const dlgBody = dialogConfig?.body ?? gooseLorem
  const dlgCancel = dialogConfig?.cancelText ?? 'Cancel'
  const dlgOkay = dialogConfig?.okayText ?? 'Okay'
  ...
  const PAD = 40 * gooseDP
  // Responsive max-width: original is phone-width fillMaxWidth; on desktop the
  // card should not stretch across the whole viewport. Clamp width and center.
  const MAX_CARD_W = 420 * gooseDP
  ...
  const card = gooseGlass(
    ...
    {
      cornerRadius: 48 * gooseDP,
      refractionHeight: 24 * gooseDP,
      refractionAmount: -48 * gooseDP,
  ...
  // The renderer's gooseLoadCSDF() is called from the gooseRender loop
  // before rendering this element (see methods-gooseRender.ts).
  ...
  const BTN_H = 48 * gooseDP
  const BTN_W = (CARD_W - 2 * 24 * gooseDP - 16 * gooseDP) / 2
  const BTN_Y = CARD_Y + CARD_H - 24 * gooseDP - BTN_H
  const CANCEL_X = CARD_X + 24 * gooseDP
  const OKAY_X = CANCEL_X + BTN_W + 16 * gooseDP
  ...
  interactions['dialog-cancel'] = { onTap: () => onDialogTap?.('cancel') }
  elements.push(
    gooseText(
      'dialog-cancel-label',
      { x: CANCEL_X, y: BTN_Y, w: BTN_W, h: BTN_H },
  ...
  interactions['dialog-okay'] = { onTap: () => onDialogTap?.('okay') }
  elements.push(
    gooseText(
      'dialog-okay-label',
      { x: OKAY_X, y: BTN_Y, w: BTN_W, h: BTN_H },
