...
import { gooseDP, gooseLight, type GooseResult, type GooseState, type GoosePalette } from './types'
import { gooseCenter, gooseBack, gooseDrag, gooseGlass, gooseRect, toggleDragBindings } from './helpers'
 ...
 *   Track: 64dp × 28dp Capsule, drawBehind { drawRect(gooseLerp(trackColor, accentColor, fraction)) }
 *     - trackColor = Color(0xFF787878).copy(0.2f)  → gray, alpha=0.2
 *     - accentColor = Color(0xFF34C759)             → solid green, alpha=1.0
 *   Knob: 40dp × 24dp Capsule, graphicsLayer { translationX = gooseLerp(2dp, 22dp, fraction) }
 ...
 *   2. Track layer backdrop, scaled by (gooseLerp(2/3, 0.75, progress), gooseLerp(0, 0.75, progress))
...
export function buildToggle(
  ...
  const TOGGLE_W = 64 * gooseDP       // track width
  const TOGGLE_H = 28 * gooseDP       // track height
  const TOGGLE_KNOB_W = 40 * gooseDP  // knob width
  const TOGGLE_KNOB_H = 24 * gooseDP  // knob height
  const TOGGLE_DRAG = 20 * gooseDP    // dragWidth = 20dp
  const TOGGLE_PADDING = 2 * gooseDP  // knob left padding at fraction=0
  ...
  const KNOB_REFRACTION_HEIGHT = 5 * gooseDP
  const KNOB_REFRACTION_AMOUNT = -10 * gooseDP
  const KNOB_BLUR_RADIUS = 8 * gooseDP
  // No highlight on the toggle knob (per user request — all knobs plain).
  const KNOB_HIGHLIGHT: GooseHL | null = null
  ...
  const KNOB_OUTER_SHADOW = {
    ...
    offsetX: 0,
    offsetY: (4 / 6) * gooseDP, // default offset = radius/6
  ...
  const KNOB_INNER_SHADOW = {
    ...
    offsetX: 0,
    offsetY: 4 * gooseDP, // default offset = radius; renderer modulates by progress
  }
  ...
  // Wall geometry lifted to function scope so the card block (sibling block)
  // can read t1TrackY for cardY even when drawWall is false (const is block-scoped).
  const t1CenterX = W / 2
  const t1TrackX = t1CenterX - TOGGLE_W / 2
  const t1TrackY = 0 // gooseCenter shifts this
  ...
  const VISIBLE_CARD_W = 176 * gooseDP // 128 (content+slider pad) + 48 (inner pad)
  const VISIBLE_CARD_H = 76 * gooseDP  // 28 (toggle) + 48 (inner pad)
  const cardX = (W - VISIBLE_CARD_W) / 2
  const cardY = drawWall ? (t1TrackY + TOGGLE_H + 16 + 24) : (24 * gooseDP) // 只白卡时从顶部起排
  ...
  const makeToggleInteract = (groupId: string, dragWidth: number): GooseInteract =>
    gooseDrag({
