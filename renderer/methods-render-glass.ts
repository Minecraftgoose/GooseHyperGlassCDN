...
/** Shared state between gooseRenderGlass and its sub-passes.
...
declare module './index' {
  interface LiquidGlassRenderer {
    ...
    gooseGlassShadow(state: GlassRenderState): void
    // gooseGlassPass and gooseGlassPost are declared in
    // their respective modules (methods-gooseRender-glass-element-pass.ts
    // and methods-gooseRender-glass-post-passes.ts).
  }
}
...
export const glassRenderMethods = {
  ...
  gooseRenderGlass(
    ...
    //   val scale = gooseLerp(1f, 1f + 16f.dp.toPx() / size.width, progress)
    //   scaleX = scaleY = scale
    ...
    //   - Container's layerBlock: scale = gooseLerp(1, 1+16dp/containerWidth, pressProgress)
    //     applied to the ENTIRE Row (including tab content).
    //   - Each tab's own graphicsLayer: scale = gooseLerp(1, 1.2, pressProgress)
    ...
    if (el.isBottomTabContent) {
      ...
      if (tg) {
        ...
        scaleX *= containerScale
        // Content's own scale (gooseLerp(1, 1.2, pressProgress)).
    ...
    // Re-enable blending after the copy (gooseCopy disables it).
    gl.enable(gl.BLEND)
    ...
    // separable Gaussian on curTex), then gooseRender the element pass sampling
    ...
    // with inlineBlurRadius=0 (handled in gooseGlassPass) since the
    // backdrop is already blurred.
    //
    // For normal elements: gooseRender the element pass directly to otherFbo
    ...
    if (el.useSeparableBlur && el.blurRadius >= 0.5) {
      ...
      // gooseBlur disables BLEND — re-enable it so gooseGlassPass
  ...
  gooseGlassShadow(this: LiquidGlassRenderer, state: GlassRenderState) {
    const gl = this.gl
