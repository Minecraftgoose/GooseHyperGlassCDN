...
import type { GooseTglState } from './types'
...
declare module './index' {
  interface LiquidGlassRenderer {
    ...
    gooseTglTarget(groupId: string, target: number): void
    gooseTglDragStart(groupId: string, startFraction: number): void
    gooseTglDrag(
      groupId: string,
    ...
    gooseTglDragEnd(groupId: string): number
    gooseSldDragEnd(groupId: string): number
     ...
     *  to drag). Unlike gooseTglTarget, this works during isDragging. */
    gooseSldDragPos(groupId: string, fraction: number): void
    gooseTglFrac(groupId: string): number
    gooseTglTargetGet(groupId: string): number
  }
}
 ...
 *   - gooseTglTarget(groupId, target): programmatic toggle (tap)
 *   - gooseTglDragStart(groupId): start a finger drag (press animation)
 *   - gooseTglDrag(groupId, startFraction, currentX, startX, dragWidth):
 *     update targetFraction based on finger delta
 *   - gooseTglDragEnd(groupId): release — snaps target to 0 or 1, returns
 *     the final value so React state can sync
 *   - gooseTglFrac(groupId): read current animated fraction (for
 *     external callers that need to know the displayed position)
 *   - gooseTglTargetGet(groupId): read current target (for drag-end threshold)
 * ------------------------------------------------------------------ */
...
export const toggleMethods = {
  ...
  gooseEnsureTgl(
    ...
    if (!st) {
      ...
      // gooseTglTarget created the group first via page.tsx toggleTargets.
   ...
   */
  gooseTglTarget(this: LiquidGlassRenderer, groupId: string, target: number) {
   ...
   */
  gooseTglDragStart(this: LiquidGlassRenderer, groupId: string, startFraction: number) {
   ...
   */
  gooseTglDragEnd(this: LiquidGlassRenderer, groupId: string): number {
   ...
   */
  gooseSldDragEnd(this: LiquidGlassRenderer, groupId: string): number {
    const st = this.toggleStates.get(groupId)
    if (!st) return 0
    st.isDragging = false
    // NO snap — keep the continuous targetFraction as-is.
    const finalTarget = st.targetFraction
    // Enable velocity tracking after drag release (same as gooseTglDragEnd).
    st.trackVelocityAfterRelease = true
    // Don't release press here — auto-release will fire when fraction
    // settles near finalTarget.
    this.gooseAnimStart()
  ...
  gooseTglFrac(this: LiquidGlassRenderer, groupId: string): number {
   ...
   */
  gooseSldDragPos(this: LiquidGlassRenderer, groupId: string, fraction: number) {
    const st = this.toggleStates.get(groupId)
    if (!st) return
  ...
  gooseTglTargetGet(this: LiquidGlassRenderer, groupId: string): number {
