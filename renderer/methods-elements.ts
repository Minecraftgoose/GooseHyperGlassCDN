...
declare module './index' {
  interface LiquidGlassRenderer {
    gooseElements(configs: GooseElement[]): void
    gooseButtons(configs: GooseElement[]): void
    gooseInterVal(id: string, value: number): void
    goosePress(id: string, pressed: boolean, position?: { x: number; y: number }): void
    gooseDragPos(id: string, position: { x: number; y: number }): void
  }
}

export const elementMethods = {
  /** Set the element list. Triggers foreground re-raster for changed elements. */
  gooseElements(this: LiquidGlassRenderer, configs: GooseElement[]) {
    this.gooseButtons(configs)
  },

  /** Set the element list (legacy name; same as gooseElements). */
  gooseButtons(this: LiquidGlassRenderer, configs: GooseElement[]) {
    ...
    for (const next of configs) {
      ...
      // gooseBtn / gooseGlass call creates a NEW array, even when
      ...
      // gooseText stores its color in text.color, but the old dirty-check only
      // compared labelColor (which gooseText always sets to [0,0,0,1]) — so
    ...
    for (const c of configs) {
      if (!this.buttonStates.has(c.id)) {
        ...
        // call gooseInterVal() right after gooseElements() to push
   ...
   */
  gooseInterVal(this: LiquidGlassRenderer, id: string, value: number) {
   ...
   */
  gooseDragPos(this: LiquidGlassRenderer, id: string, position: { x: number; y: number }) {
    const st = this.buttonStates.get(id)
