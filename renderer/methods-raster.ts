...
declare module './index' {
  interface LiquidGlassRenderer {
    gooseRaster(cfg: GooseElement): void
    gooseRasterText(cfg: GooseElement): void
    gooseUploadRaster(id: string): void
  }
}
...
export const rasterMethods = {
  gooseRaster(this: LiquidGlassRenderer, cfg: GooseElement) {
   ...
   * ---------------------------------------------------------------- */
  gooseRasterText(this: LiquidGlassRenderer, cfg: GooseElement) {
    if (!cfg.text) return
    const dpr = this.dpr
    const w = Math.max(1, Math.round(cfg.rect.w * dpr))
    const h = Math.max(1, Math.round(cfg.rect.h * dpr))
  ...
  gooseUploadRaster(this: LiquidGlassRenderer, id: string) {
    const gl = this.gl
