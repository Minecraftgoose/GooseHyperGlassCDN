...
declare module './index' {
  interface LiquidGlassRenderer {
    gooseLoadWP(src: string): Promise<void>
    gooseLoadSDF(src: string): Promise<void>
    ...
    gooseLoadCSDF(w: number, h: number, radius: number): void
    gooseResize(cssW: number, cssH: number): void
  }
}

export const wallpaperMethods = {
  /** Load the wallpaper image as a texture. */
  async gooseLoadWP(this: LiquidGlassRenderer, src: string) {
    const img = new Image()
  ...
  async gooseLoadSDF(this: LiquidGlassRenderer, src: string) {
    const img = new Image()
    img.crossOrigin = 'anonymous'
    await new Promise<void>((resolve, reject) => {
      img.onload = () => resolve()
  ...
  gooseLoadCSDF(this: LiquidGlassRenderer, w: number, h: number, radius: number) {
   ...
   */
  gooseResize(this: LiquidGlassRenderer, cssW: number, cssH: number) {
