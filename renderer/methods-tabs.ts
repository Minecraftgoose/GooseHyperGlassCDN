...
declare module './index' {
  interface LiquidGlassRenderer {
    gooseTabSel(groupId: string, tabIndex: number, tabsCount: number): void
    gooseTabDragStart(groupId: string, startTabIndex: number, tabsCount: number): void
    gooseTabDrag(
      groupId: string,
    ...
    gooseTabDragEnd(groupId: string, tabsCount: number): number
    gooseTabFrac(groupId: string): number
    gooseTabTarget(groupId: string): number
  }
}
...
export const tabsMethods = {
   ...
   */
  gooseTabDragEnd(this: LiquidGlassRenderer, groupId: string, tabsCount: number): number {
  ...
  gooseTabFrac(this: LiquidGlassRenderer, groupId: string): number {
    return this.toggleStates.get(groupId)?.fraction ?? 0
  },

  /** Read the current target tab index. */
  gooseTabTarget(this: LiquidGlassRenderer, groupId: string): number {
