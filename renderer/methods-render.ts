...
import type { GooseElement, GooseElemState } from './types'
...
export const renderMethods = {
  gooseRender(this: LiquidGlassRenderer) {
    // PERFORMANCE: Skip gooseRender if nothing changed since last frame.
    ...
    const gl = this.gl
    // Ensure FBOs exist (created lazily on first gooseRender after gooseResize).
    ...
    // by blurring fboA (wallpaper) right after gooseRenderBG, BEFORE any
    ...
    // here (once per frame) and blur fboA in-place (gooseBlur → blurFboB,
    // then gooseCopy back to fboA).
    ...
    if (sceneBlurEl) {
      ...
      // gooseBlur restored the FBO binding to fboA (what gooseRenderBG
    ...
    // Cull + iterate. We gooseRender elements in DECLARED ORDER (no Wave 1 /
    ...
    // a ping-pong; non-glass elements gooseRender directly to the current FBO.
    ...
    for (const el of this.buttonConfigs) {
      ...
      // --- Non-glass elements: gooseRender directly to current FBO ---
      if (this.gooseRenderPlain(el, r, st, curFbo)) continue

      // --- Backdrop FBO: gooseRender wallpaper+scrim+colorControls into
      ...
      // BEFORE rendering. gooseLoadCSDF() is cached — no-op if already
      ...
      // once per (w, h, radius) tuple, on the first frame after a gooseResize.
      ...
      if (el.isBottomTabContainer && this.tabsBackdropFbo && this.tabsBackdropTex) {
        ...
        // gooseCopy disables blend; re-enable for subsequent tab-content rendering.
        this.gl.enable(this.gl.BLEND)
        this.gl.blendFunc(this.gl.SRC_ALPHA, this.gl.ONE_MINUS_SRC_ALPHA)
      }
    }

    // --- Second pass: gooseRender renderOnTop elements ---
    ...
    // (handled in gooseGlassPass), so the scrim doesn't darken them.
    for (const el of this.buttonConfigs) {
      if (!el.renderOnTop) continue
      const y = el.scroll ? el.rect.y - scrollY : el.rect.y
      if (y + el.rect.h < viewportTop || y > viewportBottom) continue
      ...
      // Non-glass renderOnTop elements (scrim/dim) gooseRender directly on curFbo.
      if (this.gooseRenderPlain(el, r, st, curFbo)) continue
  ...
  gooseRenderBG(this: LiquidGlassRenderer) {
    const gl = this.gl
    this.gooseBindFBO(this.fboA)
    gl.disable(gl.BLEND)
  ...
  gooseRenderPlain(
    ...
    if (el.enterProgress != null) {
      ...
      const stretch = el.enterStretchFactor != null && derived > 1
        ? el.enterStretchFactor * (derived - 1) * 32 * gooseDP
        : 0
    ...
    if (el.kind === 'plain-rect' && el.plainRect) {
      ...
      this.gooseBindFBO(curFbo)
      // Toggle tracks: gooseLerp between offColor and onColor based on the
      // group's animated fraction. Faithful to LiquidToggle.kt's
      // `drawRect(gooseLerp(trackColor, accentColor, fraction))`.
    ...
    if (el.kind === 'text') {
      ...
      if (el.isBottomTabContent) {
        ...
        if (tg) {
          // Container scale = gooseLerp(1, 1+16dp/width, pressProgress).
