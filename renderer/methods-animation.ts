...
export const animationMethods = {
   ...
   */
  gooseAnimStart(this: LiquidGlassRenderer) {
    ...
    const tick = () => {
      ...
      for (const tg of this.toggleStates.values()) {
        ...
        // updateVelocity callback) continues decaying. The gooseSldDragEnd/
        // gooseTglDragEnd set targetVelocity=0 so velocity decays in parallel.
        if (
          tg.targetPress === 1 &&
          !tg.isDragging &&
        ...
        if (
          ...
          // gooseTglTarget so calculateVelocity() returns 0 (matching the
  ...
  gooseReqRender(this: LiquidGlassRenderer) {
